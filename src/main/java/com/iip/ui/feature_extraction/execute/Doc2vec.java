...
public class Doc2vec {
    ...
    public static String doc2vec(List<String> words, Map<String, float []> vecs) {
        // input: 字符串
...
//            String vecsPath = "src/main/java/textprocess/word2vec/mini_vec.properties";
    ...
    public static List<String> getVecDocs(List<String> docs, String vecsPath){
        ...
        for(List<String> doc: partedDocs){
            words.addAll(doc);
        }
        Map<String, float []> vecs = getvecs(words, vecsPath);
        for(List<String> doc: partedDocs){
            vecDocs.add(doc2vec(doc, vecs));
        }
        return vecDocs;
    }

    public static void main(String [] a) {
        String doc = "这里面是原味牛乳制品";
...
//        print("$"+vec.length+"#"+vec[49]);
        ...
        Participle.getPartedDocs(docs); // must ***
        for(String vecDoc: getVecDocs(docs, vecsPath)){
            print(vecDoc);
        }
    }
